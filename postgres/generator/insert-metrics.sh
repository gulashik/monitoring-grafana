...
echo "Starting metrics generation in container $CONTAINER_NAME..."

# Вызов процедуры. 
# Сама процедура содержит бесконечный цикл с pg_sleep, 
# поэтому этот вызов будет работать пока не придет сигнал прерывания.
podman exec "$CONTAINER_NAME" psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -c "call public.insert_grafana_demo_metric();"
